...
Данные распределены: оперативные сообщения в Kafka, «горячие» результаты в PostgreSQL, «холодная» аналитика в колоночном хранилище, логи аудита в отдельном защищенном storage. Это оправдано разной природой нагрузки (OLTP vs OLAP) и требованиями к хранению (immutable логи).

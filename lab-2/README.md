...
| :--- | :--- |
| **Со стороны водителя** | Один водитель управляет **нулём, одним или несколькими** маршрутами в течение рабочего периода. |
| **Со стороны маршрута** | Один маршрут выполняется **только одним** водителем (назначенным ответственным). |

#### Связь №2: `orders` (заказы) и `route_points` (точки маршрута)
| Сторона | Отношение (бизнес-правило) |
| :--- | :--- |
| **Со стороны заказа** | Один заказ может **не быть включён** ни в один маршрут (например, новый заказ) или быть включён **только в одну** точку маршрута (после оптимизации). |
| **Со стороны точки маршрута** | Одна точка маршрута (плановая остановка) соответствует **ровно одному** заказу на доставку. |
...
Схема спроектирована с расчётом на микросервисную архитектуру: каждая сущность может быть вынесена в отдельный сервис, а внешние ключи обеспечивают согласованность. 
